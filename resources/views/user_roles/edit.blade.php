...
@section('content_header')
    <h4 class="title">Edit User Roles</h4>
@endsection
@section('content')
    <div class="user-roles-form">
        {!! Form::open(array('route' => 'user-roles.update','method'=>'POST')) !!}
        @if (count($errors) > 0)
        ...
        <div class="row">
            <div class="col-md-3">
                <div class="form-group">
                    ...
                    {!! Form::select('user', count($users) > 0 ? $users : [], $user->id, ['class' => 'form-control', 'placeholder' => 'Please select user.', 'id' => 'listUser']) !!}
                </div>
            </div>
            <div class="col-md-3">
                <div class="form-group">
                    <label class="form-label">Roles</label>
                    {!! Form::select('role', $roles, $userRole ? $userRole->id: '', ['class' => 'form-control', 'placeholder' => 'Please select role.']) !!}
                </div>
            </div>
            <div class="col-md-3">
                ...
                @if( isset($permissions['administration']) && count($permissions['administration']) > 0 )
                    <ul class="checkbox-list">
                        @foreach($permissions['administration'] as $permission)
                            <li>
                                {!! Form::checkbox('permissions[]', $permission['id'], in_array($permission['id'], $userPermission), ['class' => 'material-inputs filled-in chk-col-transparent', 'id' => 'administration-' . $permission['id']]) !!}
                ...
                @if( isset($permissions['schedule']) && count($permissions['schedule']) > 0 )
                    <ul class="checkbox-list">
                        @foreach($permissions['schedule'] as $permission)
                            <li>
                                {!! Form::checkbox('permissions[]', $permission['id'], in_array($permission['id'], $userPermission), ['class' => 'material-inputs filled-in chk-col-transparent', 'id' => 'schedule-' . $permission['id']]) !!}
                ...
                @if( isset($permissions['exhumantion']) && count($permissions['exhumantion']) > 0 )
                    <ul class="checkbox-list">
                        @foreach($permissions['exhumantion'] as $permission)
                            <li>
                                {!! Form::checkbox('permissions[]', $permission['id'], in_array($permission['id'], $userPermission), ['class' => 'material-inputs filled-in chk-col-transparent', 'id' => 'exhumantion-' . $permission['id']]) !!}
                ...
                @if( isset($permissions['claimant']) && count($permissions['claimant']) > 0 )
                    <ul class="checkbox-list">
                        @foreach($permissions['claimant'] as $permission)
                            <li>
                                {!! Form::checkbox('permissions[]', $permission['id'], in_array($permission['id'], $userPermission), ['class' => 'material-inputs filled-in chk-col-transparent', 'id' => 'claimant-' . $permission['id']]) !!}
