    ...
    CalendarApp.prototype.onDrop = function(eventObj, date) {
        ...
        CalendarApp.prototype.onEventClick = function(calEvent, jsEvent, view) {
            ...
            $this.$modal.find('.delete-event').show().end().find('.save-event').hide().end().find('.modal-body').empty().prepend(form).end().find('.delete-event').unbind('click').click(function() {
                var destroyPath = $('#addBtnForm').attr('data-destroy');
            ...
            $this.$modal.find('form').on('submit', function(e) {
                e.preventDefault();
                var updatePath = $('#addBtnForm').attr('data-update');
                $.ajaxSetup({
                    headers: {
                        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                    }
                ...
                $.ajax({
                    url: updatePath,
                    method: 'post',
                    data: {'id': calEvent.id, 'name': form.find("input[type=text]").val()},
                    success: function (res) {
                        if(res.success) {
        ...
        CalendarApp.prototype.onSelect = function(start, end, allDay) {
            ...
            $this.$modal.find('form').on('submit', function() {
                ...
                var btnAddForm = $('#addBtnForm');
                var addPath = btnAddForm.attr('data-add');
                if (title !== null && title.length != 0) {
                    ...
                    $.ajax({
                        url: addPath,
                        method: 'post',
                        data: {'name': title, 'start': moment(start).format(), 'end': moment(end).format(), 'category': categoryClass},
                        success: function (res) {
...
console.log(defaultEvents);
            ...
            $this.$calendarObj = $this.$calendar.fullCalendar({
                ...
                eventClick: function(calEvent, jsEvent, view) {
                    // if( calEvent.deceased_id ) {
                    //     window.location = '/deceaseds/' + calEvent.deceased_id + '/edit';
                    // } else {
